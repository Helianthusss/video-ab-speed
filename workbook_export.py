...
def build_workbook(tabs,output_path):
    ...
    for name,table in tabs.items():
        ...
        for index,header in enumerate(headers,1):
            width=max(12,min(36,max([len(str(header))]+[len(str(r[index-1])) if r[index-1] is not None else 0 for r in table['rows'][:100]])+2))
        ...
        if name=='Raw':
            ...
            if all(k in positions for k in required):
                for row_index in range(2,ws.max_row+1):
                    ...
                    ws[f'{dt}{row_index}']=f'=IF(OR(ISBLANK({a}{row_index}),ISBLANK({b}{row_index})),"",IF({d}{row_index}="A→B",{b}{row_index}-{a}{row_index},{a}{row_index}-{b}{row_index}))'
                    ws[f'{speed}{row_index}']=f'=IF(OR({dt}{row_index}="",{dt}{row_index}<=0,{length}{row_index}<=0),"",3.6*{length}{row_index}/{dt}{row_index})'
...
def verify_workbook(path):
    ...
    if not wb.sheetnames:raise ValueError('Workbook không có sheet')
